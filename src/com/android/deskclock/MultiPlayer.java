...
public class MultiPlayer implements MediaPlayer.OnCompletionListener {
        ...
        public void setDataSource(final Uri uri) {
            ...
            if (mRandom) {
                mIsInitialized = setDataSourceImpl(mCurrentMediaPlayer, getRandomUri());
            } else {
                mIsInitialized = setDataSourceImpl(mCurrentMediaPlayer, getUriForId(mIds.get(mCurrentPosition++)));
            }
            if (mIsInitialized) {
                if (mSingle) {
                    setNextDataSource(null);
                } else if (mRandom) {
                    setNextDataSource(getRandomUri());
                } else {
                    setNextDataSource(getUriForId(mIds.get(mCurrentPosition++)));
                }
            }
        }
        ...
        private void handleSetDataSourceUri(Uri uri) {
            ...
            mExternal = rawUri.contains("external");
            ...
            do {
                mIds.add(c.getLong(columnIndex));
            } while (c.moveToNext());
        ...
        public void setLooping(boolean looping) {
            ...
            mCurrentMediaPlayer.setLooping(false);
        }
