...
public final class StopwatchCircleView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        float redPercent = (float) currentLapTime / (float) firstLapTime;

        // Draw an arc to indicate the amount of reference lap that remains.
        if (lapCount <= 1) {
            mPaint.setColor(mCircleColor);
            final float whitePercent = 1 - (redPercent > 1 ? 1 : redPercent);
            canvas.drawArc(mArcRect, 270 + (1 - whitePercent) * 360, whitePercent * 360, false,
                    mPaint);
        } else {
            redPercent = 1f;
        }
        ...
        if (lapCount > 1) {
            ...
            Log.d("MICHAEL", "sweepAngle " + sweepAngle);
