...
public class AlarmMultiPlayer implements MediaPlayer.OnCompletionListener {
     ...
     */
    public AlarmMultiPlayer(Context context) {
        mContext = context;
    }
    ...
    private Uri getNextUri() {
        ...
        if (mCursor == null) {
            LogUtils.e("AlarmMultiPlayer: mCursor = null");
            return null;
        }
    ...
    private void handleSetDataSourceUri(Uri uri) {
        ...
        if (mCursor == null) {
            LogUtils.e("AlarmMultiPlayer: Query failed, cursor = null");
            return;
        }
        mColumnIndex = mCursor.getColumnIndex(columnName);
        if (mCursor.getCount() == 0) {
            LogUtils.e("AlarmMultiPlayer: Query failed, cursor.getCount() = 0");
            return;
        }
        mCursor.moveToFirst();
