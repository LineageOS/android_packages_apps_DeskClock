...
public abstract class DigitalTimerClock extends RelativeLayout {
    ...
    public synchronized void updateTime(long elapsed) {
        ...
        if (oldTime[0] != time[0] || this.mTimerClock[0].getText().equals("")) { //$NON-NLS-1$
            this.mTimerClock[0].setText(hours);                           // Hours
            adjustSize = true;
        }
        if (oldTime[1] != time[1] || this.mTimerClock[2].getText().equals("")) { //$NON-NLS-1$
            this.mTimerClock[2].setText(String.format("%02d", time[1]));  // Minutes  //$NON-NLS-1$
        }
        if (oldTime[2] != time[2] || this.mTimerClock[4].getText().equals("")) { //$NON-NLS-1$
        ...
        if (this.mShowSign && this.mInvertColorsOnNextDraw) {
