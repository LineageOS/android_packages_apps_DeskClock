...
public abstract class DigitalTimerClock extends RelativeLayout {
    ...
    public void showEditor() throws IllegalAccessException {
        ...
        if (!this.mIsEditMode) {
            ...
            final long absMillis = Math.abs(DigitalTimerClock.this.mElapsed);
            ...
            DigitalTimerClock.this.mTimerClockEditor[0].setValue((int)hr);
            DigitalTimerClock.this.mTimerClockEditor[2].setValue((int)min);
            DigitalTimerClock.this.mTimerClockEditor[4].setValue((int)sec);
            DigitalTimerClock.this.mTimerClockEditor[6].setValue((int)ms);
    ...
    public void setEditor() throws IllegalAccessException {
        ...
        long millis = this.mTimerClockEditor[0].getValue() * 3600000L;
        millis += this.mTimerClockEditor[2].getValue() * 60000L;
        millis += this.mTimerClockEditor[4].getValue() * 1000L;
        millis += this.mTimerClockEditor[6].getValue();
    ...
    public synchronized void updateTime(long elapsed) {
        ...
        // Convert to the appropriate amount of time
        final long absMillis = Math.abs(elapsed);
        final long hr  = TimeUnit.MILLISECONDS.toHours(absMillis);
        final long min = TimeUnit.MILLISECONDS.toMinutes(absMillis
                         - TimeUnit.HOURS.toMillis(hr));
        final long sec = TimeUnit.MILLISECONDS.toSeconds(absMillis
        ...
        this.mTimerClock[2].setText(String.format("%02d", min));  // Minutes  //$NON-NLS-1$
        this.mTimerClock[4].setText(String.format("%02d", sec));  // Seconds  //$NON-NLS-1$
        this.mTimerClock[6].setText(String.format("%03d", ms));   // Milliseconds //$NON-NLS-1$
        if (this.mShowSign) {
